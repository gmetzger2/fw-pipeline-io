...
class TestSearch(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_file_entry1 = MagicMock(
            spec=FileEntry, tags=['tag1', 'tag2'])
        self.mock_file_entry2 = MagicMock(
            spec=FileEntry, tags=['tag2', 'tag3'])
        self.mock_file_entry3 = MagicMock(
            spec=FileEntry, tags=['tag1', 'tag3'])

        # Mock an aquisition container with files
        self.mock_acquisition = MagicMock(
            spec=Acquisition, files=[self.mock_file_entry1, self.mock_file_entry2, self.mock_file_entry3])

        # Mock a session container with files and an acquisition (including the
        # method iter)
        self.mock_session = MagicMock(
            spec=Session, files=[self.mock_file_entry1, self.mock_file_entry2,
                                 self.mock_file_entry3], acquisitions=MagicMock(
        ...
        self.mock_subject = MagicMock(
            spec=Subject, files=[self.mock_file_entry1, self.mock_file_entry2,
                                 self.mock_file_entry3], sessions=MagicMock(
                                     iter=MagicMock(return_value=[self.mock_session])
            )
        )
        # Mock a project container with files and a subject
        self.mock_project = MagicMock(
            spec=Project, files=[self.mock_file_entry1, self.mock_file_entry2,
                                 self.mock_file_entry3], subjects=MagicMock(
                                     iter=MagicMock(return_value=[self.mock_subject])
            )
        )
    ...
    def test_search_for_file_with_single_tag(self):
        result = self.search_instance.search_for_file_with_tag('tag1', self.mock_project)
    ...
    def test_search_for_file_with_list_of_tags_inclusive(self):
        result = self.search_instance.search_for_file_with_tag(
            tag=['tag1', 'tag2'], container=self.mock_project,
            inclusive=True,
            recursive=False
        )
    ...
    def test_search_for_file_with_list_of_tags_not_inclusive(self):
        result = self.search_instance.search_for_file_with_tag(
            ...
            inclusive=False,
            recursive=False)
        self.assertEqual(result, [self.mock_file_entry1, self.mock_file_entry2, self.mock_file_entry3])

    def test_search_for_file_recursive(self):

        # Test with recursive search
        result = self.search_instance.search_for_file_with_tag('tag1', self.mock_project, recursive=True)
